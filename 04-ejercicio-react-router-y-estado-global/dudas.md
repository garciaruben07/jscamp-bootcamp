# Aquí puedes dejar tus dudas

## Primera parte

<!-- Dudas de la primera parte del ejercicio -->

## Segunda parte

<!-- Dudas de la segunda parte del ejercicio -->

## Tercera parte

Al pasar los filtros a useSearchParams me encontré con dos cosas del código de partida que creo que
estaban mal y las he cambiado, pero no sé si era la intención. Una es que al recargar estando en la
página 3 volvía siempre a la 1, porque el ternario que lee el parámetro devolvía 1 cuando el número
era válido y NaN cuando no había parámetro. La otra es que los select no mostraban el filtro que
había en la URL al recargar, así que les he puesto defaultValue.

Si eran errores a propósito para que los encontráramos, avisadme y los dejo como estaban.

## Cuarta parte

<!-- Dudas de la cuarta parte del ejercicio -->

## Quinta parte

En el header había un enlace "Sin SPA" que recargaba la página entera. Entiendo que estaba para ver
la diferencia con el Link, y como el enunciado pedía poner los links principales con NavLink lo he
quitado y he dejado Inicio y Empleos. ¿Hice bien o lo queríais mantener?

## Sexta parte

<!-- Dudas de la sexta parte del ejercicio -->
